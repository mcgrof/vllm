...
The unit tests assert this after every eviction scenario.

## Tests

Unit tests (`tests/v1/kv_connector/unit/test_cartridge_gpu_residency.py`)
run on CPU tensors — the tier is device-agnostic — and cover:
...
5. **Multi-cartridge isolation** — per-cartridge isolation,
...
- **Tensor parallelism** — per-rank KV placement needs testing
  ...
  each TP worker will maintain its own view.
