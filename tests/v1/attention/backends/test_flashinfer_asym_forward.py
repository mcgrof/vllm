...
"""Step 1 verification: FlashInfer forward() tuple-handling helpers.
...
Also asserts the fail-closed guards: cascade, DCP prefill, TRTLLM
prefill, and TRTLLM decode must raise NotImplementedError when
handed an asymmetric tuple cache.
...
def test_prepare_symmetric_5d_returns_tensor():
...
def test_prepare_asym_returns_tuple_of_4d_tensors():
...
def test_prepare_asym_rejects_mismatched_shapes():
...
def test_prepare_asym_rejects_wrong_ndim():
...
def test_prepare_symmetric_rejects_wrong_ndim():
