...
def _util(cand: UtilityFeatures) -> float:
    return expected_utility(
        cand, MODE_RETENTION, UtilityConstants()).total
...
class TestCandidateStrictlyBetterPasses:
    def test_high_p_candidate_beats_low_p_victim(self):
        ...
        cand = UtilityFeatures(
            recency=1.0, transition_prob=0.9, transition_count=40,
        ...
        victim = UtilityFeatures(
            recency=50.0, transition_prob=0.0, transition_count=0,
            num_blocks=4, num_bytes=10 ** 8)
...
class TestHugeCandidateVsTinyVictim:
    def test_huge_cold_candidate_loses_to_tiny_hot_victim(self):
        ...
        huge_cold = UtilityFeatures(
            ...
            transition_prob=0.1, transition_count=3,
            num_blocks=50, num_bytes=4 * 10 ** 9,
        )
        tiny_hot = UtilityFeatures(
            recency=0.0, transition_prob=0.9, transition_count=50,
            num_blocks=3, num_bytes=100 * 10 ** 6,
        )
        u_cand = expected_utility(
            huge_cold, MODE_RETENTION, consts)
        u_vic = expected_utility(
            tiny_hot, MODE_RETENTION, consts)
...
class TestVictimCost:
    def test_bigger_victim_saved_ms_pushes_candidate_down(self):
        ...
        base = UtilityFeatures(
            recency=5.0, transition_prob=0.3, transition_count=5,
            num_blocks=3, num_bytes=10 ** 8,
            victim_saved_ms=0.0,
        )
    ...
    def test_zero_victim_cost_when_candidate_certain(self):
        ...
        f = UtilityFeatures(
            recency=0.0, transition_prob=1.0, transition_count=100,
            victim_saved_ms=5.0, num_blocks=1, num_bytes=0,
        )
...
class TestGateDecisionOnTotal:
    ...
    def test_hint_when_candidate_wins(self):
        cand = UtilityFeatures(
            recency=0.0, transition_prob=0.9, transition_count=40,
            num_blocks=4, num_bytes=10 ** 8)
        vic = UtilityFeatures(
            recency=40.0, num_blocks=4, num_bytes=10 ** 8)
        assert _util(cand) > _util(vic)

    def test_suppress_when_victim_wins(self):
        cand = UtilityFeatures(
            recency=30.0, transition_prob=0.05, transition_count=1,
            num_blocks=4, num_bytes=2 * 10 ** 9)
        vic = UtilityFeatures(
            recency=0.0, transition_prob=0.8, transition_count=20,
            num_blocks=4, num_bytes=10 ** 8)
        assert _util(cand) < _util(vic)
