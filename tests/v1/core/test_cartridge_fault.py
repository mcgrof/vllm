...
7. Registry fault: lookup after DB corruption / missing file.
"""
import json
import tempfile
...
def _make_valid_cache(num_layers=2, num_kv_heads=2, num_tokens=32,
                      head_dim=8, include_frozen=False):
...
class TestRegistryFaults:
    ...
    def test_register_then_delete_db_and_reopen(self):
        ...
        reg = CartridgeRegistry(db_path)
        m = CartridgeManifest(
            cartridge_id="cart_01", model_id="test",
            num_layers=2, num_kv_heads=2, head_dim=8,
            dtype="bfloat16", num_tokens_raw=32,
            num_tokens_aligned=32, block_size=16,
            num_blocks=2, has_frozen_prefix=False,
        )
        reg.register(m)
        assert reg.count() == 1
        reg.close()
