...
The tests stub out the GPU-only pieces (``triton_reshape_and_cache_flash``
...
def _build_cart_tensor(fill: float) -> torch.Tensor:
    ...
    t = torch.full(
        (2, TOKENS_PER_CART, NUM_KV_HEADS, HEAD_DIM), fill,
        dtype=torch.float32,
    )
    return t
...
def _build_meta(requests: list[tuple[str, str, torch.Tensor, int]]
                ) -> CartridgeConnectorMetadata:
    ...
    for _, cart_id, slots, n in requests:
        meta.requests.append(CartridgeReqMeta(
            cartridge_id=cart_id, slot_mapping=slots, num_tokens=n,
        ))
    return meta
    ...
    def test_two_requests_two_cartridges_no_crosscontamination(self):
        ...
        try:
            for req in meta.requests:
                src = store.get(
                    SimpleNamespace(
                        cartridge_id=req.cartridge_id, layer_idx=0)
                )
                # Loop layers
                for layer_idx, kv_layer in enumerate(kv_cache_layers):
                    ...
                    flat_k = key_cache.reshape(
                        -1, NUM_KV_HEADS, HEAD_DIM)
                    flat_v = value_cache.reshape(
                        -1, NUM_KV_HEADS, HEAD_DIM)
        ...
        for layer_idx, kv_layer in enumerate(kv_cache_layers):
            ...
            assert torch.all(key_cache[slots_b] == 2.0), (
                f"layer {layer_idx}: B-slots got non-B data "
                "(cross-contamination!)"
            )
            assert torch.all(val_cache[slots_b] == 2.0)
            # Slots outside both ranges remain zero.
            other_slots = torch.tensor(
                [s for s in range(num_blocks * BLOCK_SIZE)
                 if s not in slots_a.tolist()
                 and s not in slots_b.tolist()],
                dtype=torch.long,
            )
...
class TestInjectionKernelPerRequest:
    ...
    def test_independent_writes_do_not_stomp(self):
        ...
        # Stub triton_reshape_and_cache_flash with an in-process
        ...
        with patch(
            ...
            inject_kv_into_paged_cache(
                src_key=k_a, src_value=v_a,
                kv_cache_layer=kv_cache_layer,
                slot_mapping=slots_a,
            )
            inject_kv_into_paged_cache(
                src_key=k_b, src_value=v_b,
                kv_cache_layer=kv_cache_layer,
    ...
    def test_router_result_becomes_metadata_id(self):
        ...
        req_a = SimpleNamespace(
            ...
            sampling_params=SimpleNamespace(
                extra_args={"cartridge_id": "patient_00"}),
        )
        req_b = SimpleNamespace(
            request_id="b",
            sampling_params=SimpleNamespace(
                extra_args={"cartridge_id": "patient_01"}),
        )
