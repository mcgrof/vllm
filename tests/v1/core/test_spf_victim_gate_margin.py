...
def _cand(rid_tail: int,
    ...
    r = ResourceId.for_prefix(list(range(16)) + [rid_tail], session_id=session)
    r = ResourceId(
        scope=r.scope,
        ...
        session_id=session,
        first_block_hash=r.first_block_hash,
    )
...
class TestMarginZeroPreservesPhase2:
    ...
    def test_strict_greater_than_at_margin_zero(self):
        ...
        victim_rid = ResourceId.for_prefix(list(range(16)) + [99],
                                           session_id="s0")
        victim_rid = ResourceId(
            scope=victim_rid.scope,
            resource_id=victim_rid.resource_id,
            num_blocks=1,
            num_bytes=victim_rid.num_bytes,
            session_id="s0",
...
class TestMarginPositiveSuppresses:
    ...
    def test_small_edge_suppressed_with_margin(self):
        ...
        cand = _cand(1)
        c.observe_resource(cand.resource)
        victim_rid = ResourceId.for_prefix(list(range(16)) + [100],
                                           session_id="s0")
        victim_rid = ResourceId(
            scope=victim_rid.scope,
            resource_id=victim_rid.resource_id,
    ...
    def test_clear_edge_still_admitted_with_margin(self):
        ...
        v_rid = ResourceId.for_prefix(list(range(16)) + [88], session_id="s0")
        v_rid = ResourceId(
            scope=v_rid.scope,
            resource_id=v_rid.resource_id,
            num_blocks=1,
            num_bytes=v_rid.num_bytes,
            session_id="s0",
            first_block_hash=v_rid.first_block_hash,
        )
        ...
        for _ in range(30):
            c.step_candidates(candidates=[], free_gpu_blocks=0, victim=None)
        cand = _cand(2)
        c.observe_resource(cand.resource)
...
class TestMarginBackCompat:
    ...
    def test_env_parses(self):
        ...
        with patch.dict(os.environ, {"VLLM_SPF_VICTIM_GATE_MARGIN_MS": "0.1"},
                        clear=False):
