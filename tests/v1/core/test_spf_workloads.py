...
@pytest.mark.parametrize("name,fn,params", [
    ...
    ("batched_burst", batched_burst, dict(n_bursts=3, burst_size=5, seed=42)),
    ("shared_prefix", shared_prefix, dict(n_events=30, seed=42)),
    ("conversation_tree", conversation_tree,
     dict(n_conversations=2, depth=3, branches=2, seed=42)),
    ("random_no_reuse", random_no_reuse, dict(n_events=30, seed=42)),
    ("false_shared_first_block", false_shared_first_block,
     dict(n_events=30, seed=42)),
    ("long_doc_qa", long_doc_qa, dict(n_docs=3, queries_per_doc=5, seed=42)),
])
...
@pytest.mark.parametrize("fn,params", [
    ...
    (conversation_tree, dict(n_conversations=2, depth=3, branches=2, seed=1)),
    (random_no_reuse, dict(n_events=30, seed=1)),
    (false_shared_first_block, dict(n_events=30, seed=1)),
    (long_doc_qa, dict(n_docs=3, queries_per_doc=5, seed=1)),
])
...
class TestMixedSessionInterleave:
    ...
    def test_small_set_of_recurring_prefixes(self):
        events = mixed_session_interleave(n_sessions=4,
                                          n_prefixes=5,
                                          n_events=50,
                                          prefix_len=32,
                                          tail_len=8,
    ...
    def test_sessions_interleave(self):
        events = mixed_session_interleave(n_sessions=3, n_events=30, seed=0)
...
class TestBatchedBurst:
    ...
    def test_one_session_owns_each_burst(self):
        events = batched_burst(n_sessions=3, burst_size=10, n_bursts=4, seed=0)
        # Within a single burst, all events have the same session.
        for burst_idx in range(4):
            burst = [e for e in events if e.meta["burst_idx"] == burst_idx]
            assert len(burst) == 10
            sessions = {e.session_id for e in burst}
            assert len(sessions) == 1

    def test_sessions_rotate_across_bursts(self):
        events = batched_burst(n_sessions=3, burst_size=5, n_bursts=6, seed=0)
        # Bursts 0-5 map to sessions [s0, s1, s2, s0, s1, s2].
        by_burst = {}
        for e in events:
            by_burst.setdefault(e.meta["burst_idx"], set()).add(e.session_id)
...
class TestSharedPrefix:
    ...
    def test_all_events_share_identical_prefix(self):
        events = shared_prefix(n_events=25, shared_len=64, tail_len=16, seed=0)
        first_prefix = events[0].token_ids[:64]
        for e in events:
            assert e.token_ids[:64] == first_prefix

    def test_tails_diverge(self):
        events = shared_prefix(n_events=10, shared_len=32, tail_len=16, seed=0)
...
class TestConversationTree:
    ...
    def test_children_extend_parent(self):
        ...
        events = conversation_tree(n_conversations=1,
                                   depth=4,
                                   branches=2,
                                   segment_len=8,
                                   seed=0)
        ...
        for conv_events in by_conv.values():
            for i, e in enumerate(conv_events):
                ...
                ancestors = [
                    ...
                    and e.token_ids[:len(prev.token_ids)] == prev.token_ids
                ]
                assert ancestors, (f"event at depth {e.meta['depth']} has no "
                                   f"ancestor prefix")

    def test_depth_structure(self):
        events = conversation_tree(n_conversations=2,
                                   depth=3,
                                   branches=2,
                                   segment_len=4,
                                   seed=0)
...
class TestRandomNoReuse:
    ...
    def test_every_prefix_unique(self):
        events = random_no_reuse(n_events=50, prefix_len=64, seed=0)
...
class TestShuffledControl:
    ...
    def test_order_actually_changes(self):
        ...
        preserved = sum(1 for a, b in zip(base, shuf)
...
class TestFalseSharedFirstBlock:
    ...
    def test_all_share_first_block(self):
        events = false_shared_first_block(n_events=30,
    ...
    def test_honest_identity_distinguishes_all(self):
        ...
        events = false_shared_first_block(n_events=30, seed=0)
        ids = {hash_prefix_tokens(e.token_ids) for e in events}
        assert len(ids) == 30
...
class TestLongDocQA:
    ...
    def test_introduces_each_doc_once(self):
        ...
        intros = [e for e in events if e.meta.get("phase") == "intro"]
        assert len(intros) == 4
    ...
    def test_query_count_matches_param(self):
        ...
        queries = [e for e in events if e.meta.get("phase") == "query"]
    ...
    def test_hit_ratio_respected_statistically(self):
        ...
        variance is tight."""
        events = long_doc_qa(n_docs=5,
                             queries_per_doc=100,
                             hit_ratio=0.8,
                             seed=0)
        queries = [e for e in events if e.meta.get("phase") == "query"]
        n_hit = sum(1 for e in queries if e.meta["hit_miss"] == "hit")
    ...
    def test_hit_query_reuses_doc_prefix(self):
        events = long_doc_qa(n_docs=2,
                             doc_len=128,
                             queries_per_doc=10,
                             hit_ratio=1.0,
                             seed=0)
        intros = {
            e.meta["doc_id"]: e.token_ids
