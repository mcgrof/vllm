...
class TestResidencyBasics:
    ...
    def test_acquire_unknown_cartridge_raises(self):
        ...
        with pytest.raises(GPUResidencyError, match="not loaded in backing"):
...
class TestRefcountSafety:
    ...
    def test_eviction_blocked_while_pinned(self):
        ...
        mgr = GPUResidencyManager(
            store,
            capacity_bytes=int(1.5 * _chunk_bytes()),
            device="cpu",
        )
        mgr.acquire("A")
...
class TestPinnedProtection:
    ...
    def test_cartridge_larger_than_capacity_raises(self):
        ...
        mgr = GPUResidencyManager(
            store,
            capacity_bytes=_chunk_bytes() // 2,
            device="cpu",
        )
        with pytest.raises(GPUResidencyError, match="will never fit"):
            mgr.acquire("A")
...
class TestMetricsCoverage:
    ...
    def test_wait_on_promotion_counter(self):
        ...
        assert mgr.snapshot_metrics()["request_wait_on_promotion_count"] == 3
...
class TestThreadSafety:
    def test_concurrent_acquires_and_releases(self):
        ...
        threads = [
            threading.Thread(target=worker, args=(cid, 50)) for cid in ["A", "B", "C"]
        ]
        for t in threads:
            t.start()
        for t in threads:
