...
class TestLookupByLabel:
    def test_find_by_patient_id(self, registry):
        registry.register(
            _make_manifest("cart_p01", labels={"patient_id": "patient_01"})
        )
        registry.register(
            _make_manifest("cart_p02", labels={"patient_id": "patient_02"})
        )
        registry.register(
            _make_manifest(
    ...
    def test_no_match(self, registry):
        registry.register(
            _make_manifest("cart_01", labels={"patient_id": "patient_01"})
        )
...
class TestLookupByModel:
    def test_find_by_model(self, registry):
        registry.register(
            _make_manifest("cart_llama", model_id="meta-llama/Llama-3.2-3B-Instruct")
        )
        registry.register(
            _make_manifest("cart_qwen", model_id="Qwen/Qwen2.5-7B-Instruct")
        )
