...
cartridge KV data — that is handled by the CartridgeStore.
"""
...
class CartridgeRegistry:
    ...
    def lookup(self, cartridge_id: str) -> CartridgeManifest | None:
    ...
    def lookup_by_label(self, key: str, value: str) -> list[CartridgeManifest]:
        ...
        return [
            CartridgeManifest.from_dict(json.loads(r["manifest_json"])) for r in rows
        ]
    ...
    def list_all(self) -> list[CartridgeManifest]:
        ...
        rows = self._conn.execute("SELECT manifest_json FROM cartridges").fetchall()
        return [
            CartridgeManifest.from_dict(json.loads(r["manifest_json"])) for r in rows
        ]
    ...
    def count(self) -> int:
        ...
        row = self._conn.execute("SELECT COUNT(*) as n FROM cartridges").fetchone()
        return row["n"]
