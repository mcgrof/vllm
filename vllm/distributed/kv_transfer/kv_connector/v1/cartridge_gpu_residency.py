    ...
    def snapshot(self) -> dict[str, int]:
        return {
            ...
            "request_wait_on_promotion_count":
                self.request_wait_on_promotion_count,
        }
...
class GPUResidencyManager:
    ...
    def __init__(
        ...
        if eviction_policy != "lru":
            raise ValueError(
                f"only 'lru' eviction is implemented, "
                f"got {eviction_policy!r}")

        self._store = store
        self._capacity_bytes = int(capacity_bytes)
        self._device = torch.device(device) if isinstance(
        ...
        self._entries: "OrderedDict[str, _CartridgeGPUEntry]" = (
            OrderedDict())
        ...
        logger.info(
            "GPUResidencyManager: device=%s capacity=%d bytes "
            "(%.2f GB) policy=%s",
            self._device, self._capacity_bytes,
            self._capacity_bytes / 1e9, self._eviction_policy,
        )
    ...
    def acquire(
        ...
        with self._lock:
            ...
            required_bytes = sum(
                t.nelement() * self._element_size(t, dtype)
                for t in chunks_cpu.values()
            )
            ...
            for layer_idx, cpu_tensor in sorted(chunks_cpu.items()):
                ...
                tensor_bytes = (
                    gpu_tensor.nelement() * gpu_tensor.element_size())
            ...
            logger.info(
                "GPU promote: cartridge=%s chunks=%d bytes=%d "
                "resident=%d/%d",
                cartridge_id, len(new_entry.chunks),
                new_entry.bytes, self._bytes_resident,
    ...
    def release(self, cartridge_id: str) -> None:
        ...
        with self._lock:
            ...
            if entry is None:
                logger.warning(
                    "release: cartridge %s not resident; "
                    "nothing to release", cartridge_id,
                )
                return
    ...
    def get_chunk(
        ...
        with self._lock:
            ...
            if entry is None:
                raise GPUResidencyError(
                    f"cartridge {cartridge_id!r} not resident; "
                    f"call acquire() first"
                )
    ...
    def prefetch(
        ...
        with self._lock:
            ...
            self.metrics.cpu_hit += len(chunks_cpu)
            logger.info(
                "GPU prefetch: cartridge=%s chunks=%d bytes=%d "
                "(not pinned)",
                cartridge_id, len(new_entry.chunks),
                new_entry.bytes,
            )
    ...
    def snapshot_metrics(self) -> dict[str, int]:
        with self._lock:
            ...
            snap["current_gpu_resident_cartridges"] = len(
                self._entries)
    ...
    def _fetch_cpu_chunks(
        ...
        if residency is None:
            raise GPUResidencyError(
                f"cartridge {cartridge_id!r} not loaded in "
                f"backing CartridgeStore"
            )
    ...
    def _evict_until_fits(
        ...
        """
        if not self._try_evict_until_fits(
            needed_bytes, incoming_cartridge_id,
        ):
    ...
    def _try_evict_until_fits(
        ...
        for victim_id in list(self._entries.keys()):
            ...
            logger.info(
                "GPU evict: cartridge=%s freed=%d bytes "
                "(new resident=%d/%d)",
                victim_id, freed, self._bytes_resident,
                self._capacity_bytes,
            )
