...
"""
CartridgeManifest: metadata for a single learned KV-prefix resource.

Every cartridge or ReasonCACHE prefix has a manifest that describes what
model it was trained for, its shape parameters, and optional
routing/application metadata.
...
class CartridgeManifest:
    ...
    # === Model compatibility ===
    model_id: (
        str  # HuggingFace model ID (e.g., "meta-llama/Llama-3.2-3B-Instruct")
    )
    num_layers: int
    num_kv_heads: int
    head_dim: int
    ...
    resource_kind: str = "cartridge"  # "cartridge", "reasoncache", "kv_prefix"
    source_format: str = "trainable_cache"  # loader-level checkpoint format
    kv_layout: str = ""  # original layer layout hint, if supplied
    ...
    def validate_against_model(
        ...
        if self.model_id != model_id:
            errors.append(f"model_id mismatch: cartridge={self.model_id}, "
                          f"server={model_id}")
        if self.num_layers != num_layers:
            errors.append(f"num_layers mismatch: cartridge={self.num_layers}, "
                          f"model={num_layers}")
        ...
        if self.head_dim != head_dim:
            errors.append(f"head_dim mismatch: cartridge={self.head_dim}, "
                          f"model={head_dim}")
        return errors
    ...
    def validate_against_block_size(self, block_size: int) -> list[str]:
        ...
        if self.block_size != block_size:
            errors.append(f"block_size mismatch: cartridge={self.block_size}, "
                          f"server={block_size}")
        return errors
    ...
    def from_cartridge(
        ...
        """
        from .cartridge_connector import align_to_block_size, load_cartridge
        ...
        # Infer dtype from the normalized first-layer K tensor.
        first_k = cartridge["kv_data"][0][0] if cartridge["kv_data"] else None
        dtype_str = (str(first_k.dtype).replace("torch.", "")
                     if first_k is not None else "unknown")
        source_format = cartridge.get("source_format", "unknown")
        ...
        return cls(
            ...
            has_frozen_prefix=cartridge.get("has_frozen_prefix", False),
            num_frozen_tokens=cartridge.get("num_frozen_tokens", 0),
            resource_kind=resource_kind,
            source_format=source_format,
            kv_layout=cartridge.get("kv_layout", ""),
