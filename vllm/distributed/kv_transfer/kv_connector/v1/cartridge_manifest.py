...
class CartridgeManifest:
    ...
    def validate_against_model(
        ...
        if self.model_id != model_id:
            errors.append(
                f"model_id mismatch: cartridge={self.model_id}, server={model_id}"
            )
        if self.num_layers != num_layers:
            errors.append(
                f"num_layers mismatch: cartridge={self.num_layers}, model={num_layers}"
            )
        ...
        if self.head_dim != head_dim:
            errors.append(
                f"head_dim mismatch: cartridge={self.head_dim}, model={head_dim}"
            )
        return errors
    ...
    def validate_against_block_size(self, block_size: int) -> list[str]:
        ...
        if self.block_size != block_size:
            errors.append(
                f"block_size mismatch: cartridge={self.block_size}, server={block_size}"
            )
        return errors
    ...
    def from_cartridge(
        ...
        dtype_str = (
            str(first_k.dtype).replace("torch.", "")
