...
class CartridgeConnector(KVConnectorBase_V1):
    ...
    def __init__(
        ...
        if manifest_path is not None:
            from vllm.distributed.kv_transfer.kv_connector.v1.cartridge_manifest import (  # noqa: E501
                CartridgeManifest,
            )
        ...
        self._store.load(self._cartridge_id, cartridge_path, manifest, device="cpu")
        ...
        logger.info(
            "Cartridge loaded via store: id=%s, %d layers, %d tokens (%d blocks of %d)",
            self._cartridge_id,
            self._num_layers,
    ...
    def get_num_new_matched_tokens(
        ...
        # the request has.  Also ensure at least one token remains
        # for the scheduler to process (it asserts num_new_tokens > 0).
        max_claim = len(prompt_ids) - 1
        if max_claim <= 0:
            return 0, False
        matched = align_to_block_size(
            min(self._num_cartridge_tokens, max_claim),
            self._block_size,
        )
