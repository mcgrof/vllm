...
def inject_kv_into_paged_cache(
    ...
    Args:
        ...
        kv_cache_layer: the paged cache for one layer. Either the raw
            cache tensor with K/V stacked at dim 0 or dim 1, or a
        ...
        attn_layer: optional Attention module owning
            ``kv_cache_layer``. When provided and its impl exposes
            the standard ``do_kv_cache_update`` hook, the write is
            routed through the backend's own cache-update path so the
            byte layout matches the backend's read layout by
            construction. When None, falls back to the flat
            token-major write, which is only correct for flat-layout
            backends.
    """
    ...
    # HIP-swizzled behind a nominally flat allocation
    # (PagedAttention.split_kv_cache views), so a flat write into it
    # corrupts silently — injected tokens are read scrambled while
    # natively computed ones stay fine. Routing the write through the
    # impl's do_kv_cache_update — the exact call the backend uses for
    # natively computed tokens — makes the write layout match the
    # read layout by construction, for any backend. MLA impls are
    # excluded (their do_kv_cache_update takes (kv_c_normed, k_pe),
    ...
    # Flat-write fallback: auto-detect the K/V split dimension.
    #
    ...
    #       — K-only; V lives in a sibling tensor and kv_cache_layer
    #         is passed as a (k_cache, v_cache) tuple/list.
    if isinstance(kv_cache_layer, (tuple, list)) and len(kv_cache_layer) == 2:
        ...
        desc = (
            ...
            if hasattr(kv_cache_layer, "shape")
            else type(kv_cache_layer)
        )
        raise ValueError(
            f"Cannot determine K/V split for cache shape {desc}."
...
class CartridgeConnector(KVConnectorBase_V1):
    ...
    * **Multi-cartridge** (``cartridges`` + ``router``): multiple
    ...
    def _load_cartridges_from_config(self) -> None:
        ...
        self._default_cartridge_id: str | None = None
        ...
        if cartridges_list:
            ...
            if self._default_cartridge_id is None:
                ...
                self._default_cartridge_id = first_entry.get("cartridge_id") or next(
    ...
    def _load_one(
        ...
        explicit_cartridge_id: str | None,
    ) -> str:
        ...
        if manifest_path is not None:
            ...
            errors.extend(manifest.validate_against_block_size(self._block_size))
            if errors:
                raise ValueError(
                    "Cartridge manifest validation failed:\n"
                    + "\n".join(f"  - {e}" for e in errors)
                )
            logger.info("Cartridge manifest validated: %s", manifest.cartridge_id)
            if (
                explicit_cartridge_id is not None
        ...
        logger.info(
            "Cartridge loaded: id=%s, %d layers, %d tokens (%d blocks of %d)",
            cart_id,
    ...
    def _build_residency(self) -> GPUResidencyManager:
        ...
        if explicit is not None:
            ...
            for cart_id in self._cartridge_meta:
                ...
                bytes_est = res.num_layers * 2 * res.num_tokens * heads * dim * 4
    ...
    def _preload_cartridges_if_configured(self) -> None:
        ...
        """
        preload = self._kv_transfer_config.get_from_extra_config("preload", None)
        if not preload:
            return
    ...
    def _build_router_from_config(self) -> CartridgeRouter:
        ...
        """
        router_cfg = self._kv_transfer_config.get_from_extra_config("router", None)
    ...
    def start_load_kv(self, forward_context: "ForwardContext", **kwargs: Any) -> None:
        ...
        for cart_id in unique_ids:
            try:
                ...
                logger.error(
                    "start_load_kv: cannot acquire cartridge %r on GPU tier: %s",
                    cart_id,
                    e,
                )
        ...
        try:
            ...
            for request in metadata.requests:
                ...
                if cart_id not in acquired_ids:
                    continue
                num_layers = self._cartridge_meta.get(cart_id, {}).get("num_layers")
                if num_layers is None:
                    logger.error(
                        "start_load_kv: unknown cartridge_id %r; skipping request",
                        cart_id,
                    )
                    continue
    ...
    def _peek_target_device_dtype(
        ...
        for layer_name in forward_context.no_compile_layers:
            ...
            if kv_cache_attr is None:
                continue
            # Same binding rules as _inject_request: a raw tensor is
            # the whole cache and must not be indexed.
            if isinstance(kv_cache_attr, torch.Tensor):
                if kv_cache_attr.numel() == 0:
                    continue
                kv = kv_cache_attr
            else:
    ...
    def _inject_request(
        ...
        for layer_name in forward_context.no_compile_layers:
            ...
            # Backends bind layer.kv_cache either as the raw cache
            # tensor (current trees), as [cache_tensor] (older list
            # binding), or as [k_cache, v_cache] (len=2, both 4D
            # separate tensors). A raw tensor must be passed whole:
            # indexing [0] on it yields an accidental (K, V)-looking
            # pair for a (2, num_blocks, ...) allocation or a
            # block-0 slice for a (num_blocks, 2, ...) one — both
            # wrong.
            if isinstance(kv_cache_attr, torch.Tensor):
                if kv_cache_attr.numel() == 0:
                    continue
                ...
                len(kv_cache_attr) == 2
                and hasattr(kv_cache_attr[0], "dim")
                and kv_cache_attr[0].dim() == 4
            ):
