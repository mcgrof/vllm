...
def _key_to_chunk(key: CacheEngineKey) -> ChunkKey | None:
...
class CartridgeLMCachePlugin(StoragePluginInterface):
    ...
    def __init__(
        ...
        local_cpu_backend: Any | None = None,
        ...
        self._store: CartridgeStore | None = None
    ...
    def get_blocking(self, key: CacheEngineKey) -> Any | None:
        if self._store is None:
            return None
        chunk = _key_to_chunk(key)
        if chunk is None:
        ...
        # LMCache expects specific MemoryObj subtypes here; we return
        # the raw tensor instead. The connector's inject path handles
        # the tensor directly, and non-cartridge callers never reach
        # this plugin (miss for non-matching keys).
        return tensor
