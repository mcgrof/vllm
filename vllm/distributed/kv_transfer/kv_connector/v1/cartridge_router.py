...
def _get_extras_value(request: Request, key: str) -> str | None:
    """Look up a key in the request's connector metadata.

    Checks ``request.kv_transfer_params`` first (the canonical vLLM
    location for connector-bound request metadata — the V1 Request
    populates it from ``extra_args["kv_transfer_params"]``), then the
    raw extras top level, then the nested ``kv_transfer_params`` dict
    for callers that hand-build requests.
    """
...
class LabelCartridgeRouter(CartridgeRouter):
    ...
    def resolve(self, request: Request) -> str | None:
        ...
        if label_value is None:
            return None
        matches = self._registry.lookup_by_label(self._label_key, label_value)
        if not matches:
            logger.debug(
...
def build_router_from_config(
    ...
    if router_type == "label":
        if registry is None:
            raise ValueError("LabelCartridgeRouter requires a CartridgeRegistry")
    ...
    if router_type == "static":
        ...
        if not cart_id:
            raise ValueError(
                "static router requires 'cartridge_id' (or a default_cartridge_id)"
            )
