...
class ChunkKey:
    ...
    def __eq__(self, other):
        if not isinstance(other, ChunkKey):
            return False
        return (
            self.cartridge_id == other.cartridge_id
...
class CartridgeStore:
    ...
    def load(
        ...
        with self._lock:
            if cartridge_id in self._residency:
                raise ValueError(
                    f"Cartridge {cartridge_id} is already loaded. Call evict() first."
                )

        cartridge = load_cartridge(cartridge_path)
        num_tokens = align_to_block_size(cartridge["num_tokens"], self._block_size)
        ...
        logger.info(
            ...
            cartridge_id,
            cartridge["num_layers"],
            num_tokens,
            device,
        )
        return chunks_loaded

    def get(self, key: ChunkKey) -> torch.Tensor | None:
    ...
    def get_all_layers(self, cartridge_id: str) -> list[torch.Tensor] | None:
        ...
        with self._lock:
            ...
            if res is None:
                return None
            return [
                self._chunks[ChunkKey(cartridge_id, li)] for li in range(res.num_layers)
            ]
    ...
    def get_residency(self, cartridge_id: str) -> CartridgeResidency | None:
