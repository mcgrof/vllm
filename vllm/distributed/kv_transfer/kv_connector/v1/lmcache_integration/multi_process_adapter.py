...
def striding_block_hashes(block_hashes: list[bytes],
                          blocks_in_chunk: int) -> Iterable[bytes]:
...
def send_lmcache_request(
    ...
    future = mq_client.submit_request(request_type, payloads,
...
class LMCacheMPSchedulerAdapter:
    ...
    def _create_hash_key(self,
                         chunk_hash: bytes,
                         request_id: str | None = None) -> IPCCacheEngineKey:
...
class LMCacheMPWorkerAdapter:
    ...
    def __init__(
        ...
        self.store_futures: dict[str, tuple[MessagingFuture[StoreResult],
                                            list[str]]] = {}
        self.retrieve_futures: dict[str, tuple[MessagingFuture[RetrieveResult],
                                               list[str]]] = {}
    ...
    def is_first_rank_of_pp_group(self) -> bool:
        ...
        return (self.parallel_strategy.actual_worker_id %
                self.parallel_strategy.tp_size == 0)
    ...
    def submit_store_request(self, request_id: str, op: LoadStoreOp,
        ...
        future = send_lmcache_request(
            self.mq_client,
            RequestType.STORE,
            [keys, self.instance_id, op.block_ids,
             event.ipc_handle()],
        ).to_cuda_future()
    ...
    def batched_submit_store_requests(
        ...
        for request_id, op in zip(request_ids, ops, strict=False):
            if op.block_hashes is not None:
                chunk_hashes = list(
                    striding_block_hashes(op.block_hashes,
    ...
    def get_finished(
        ...
        for request_id, (r_future,
                         other_reqs) in self.retrieve_futures.items():
    ...
    def shutdown(self):
        ...
        send_lmcache_request(self.mq_client, RequestType.UNREGISTER_KV_CACHE,
