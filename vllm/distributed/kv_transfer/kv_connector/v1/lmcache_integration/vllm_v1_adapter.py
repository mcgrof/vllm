...
from vllm.distributed.kv_transfer.kv_connector.v1.lmcache_integration.utils import (
    ENGINE_NAME, apply_mm_hashes_to_token_ids, create_lmcache_metadata,
    extract_mm_features, lmcache_get_or_create_config, mla_enabled)
...
class ReqMeta:
    ...
    request_configs: dict | None = None
    # Optional routing-prior chunk subset. When set, `start_load_kv`
    # dispatches to `LMCacheEngine.retrieve_chunks` with the chunks at
    # these positional indices (i.e. chunk i in the chunk-aligned walk
    # over `token_ids`) instead of the full prefix `retrieve`. This is
    # the A.2b.1 sparse-fetch path — the saving is exactly K/N of the
    # per-chunk SSD reads against LocalDiskBackend.
    selected_chunk_indices: list[int] | None = None
    ...
    def from_request_tracker(
        ...
        request_skip = (tracker.request_configs
                        or {}).get("lmcache.skip_save", False)

        skip_save = tracker.disagg_spec is None and (
            tracker.skip_save or
        ...
        num_tokens_to_save = ((input_token_len // lmcache_chunk_size *
                               lmcache_chunk_size) if not is_last_prefill
                              or discard_partial_chunks else input_token_len)
        ...
        if tracker.mm_hashes:
            ...
            apply_mm_hashes_to_token_ids(token_ids_tensor, tracker.mm_hashes,
        ...
        slot_mapping = (block_offsets.reshape(
            (1, block_size)) + block_ids.reshape((num_blocks, 1)) * block_size)
        ...
        # A.2b.1: routing-prior chunk subset. Accepted via request_configs
        # under the key "lmcache.selected_chunk_indices" (list[int]).
        # The caller attaches this by setting
        # `sampling_params.extra_args["kv_transfer_params"]
        #  ["lmcache.selected_chunk_indices"] = [...]`.
        # `extract_request_configs` already copies any "lmcache.*"
        # key through, so no additional plumbing is needed.
        selected_chunk_indices: list[int] | None = None
        if tracker.request_configs is not None:
            sel = tracker.request_configs.get("lmcache.selected_chunk_indices")
            if sel is not None:
                # Normalize to list[int].
                selected_chunk_indices = [int(i) for i in sel]
...
def _init_lmcache_engine(
    ...
    assert isinstance(
        lmcache_config,
        LMCacheEngineConfig), ("LMCache v1 configuration is should be passed.")

    kv_dtype = get_kv_cache_torch_dtype(cache_config.cache_dtype,
                                        model_config.dtype)

    use_mla = mla_enabled(model_config)
    if use_mla and (lmcache_config.remote_serde != "naive"
    ...
    kv_connector_extra_config = getattr(kv_transfer_config,
...
class LMCacheConnectorV1Impl:
    ...
    def __init__(
        ...
        if kv_connector_extra_config:
            for key, value in kv_connector_extra_config.items():
                if key.startswith("lmcache."):
                    ...
                    if validate_and_set_config_value(config, config_key,
                                                     value):
                        logger.info(
        ...
        self.layerwise_retrievers: list[Generator[torch.Tensor | None, None,
                                                  None]] = []
        self._stats_monitor = LMCStatsMonitor.GetOrCreate()
        if role == KVConnectorRole.SCHEDULER:
            # Create lookup client using factory.
            # Build a scheduler-side LMCacheMetadata for the lookup
            # client. The scheduler may run on CPU-only nodes, so use
            # world_size for local_world_size (same convention as the
            # lmcache-bundled adapter).
            sched_metadata, _ = create_lmcache_metadata(
                vllm_config=vllm_config)
            sched_metadata.engine_id = getattr(vllm_config.kv_transfer_config,
                                               "engine_id", None)
            sched_metadata.kv_connector_extra_config = getattr(
                vllm_config.kv_transfer_config,
                "kv_connector_extra_config",
                None,
            )
            self.lookup_client = LookupClientFactory.create_lookup_client(
                config, sched_metadata)
            ...
            self.lookup_server = LookupClientFactory.create_lookup_server(
                self.lmcache_engine, self.lmcache_engine.metadata)
    ...
    def get_kv_events(self):
        """Return KV cache store events for metrics."""
        ...
        if (vllm_config.parallel_config.data_parallel_size_local == 1
            ...
            self.plugin_launcher = RuntimePluginLauncher(
                ...
                role,
                self.worker_count,
                -1 if self.lmcache_engine is None  # scheduler side
    ...
    def get_inference_info(self) -> dict:
        ...
        inference_info = {
            ...
            "model_config": {
                ...
                "dtype":
                str(getattr(vllm_config.model_config, "dtype", None)),
                "max_model_len":
                getattr(vllm_config.model_config, "max_model_len", None),
                "vocab_size":
                vllm_config.model_config.get_vocab_size(),
                "num_layers":
                getattr(vllm_config.model_config, "get_num_layers",
                        lambda _: None)(vllm_config.parallel_config),
                "num_attention_heads":
                getattr(vllm_config.model_config, "get_num_attention_heads",
                        lambda _: None)(vllm_config.parallel_config),
                "num_kv_heads":
                getattr(vllm_config.model_config, "get_num_kv_heads",
                        lambda _: None)(vllm_config.parallel_config),
                "head_size":
                getattr(vllm_config.model_config, "get_head_size",
                        lambda: None)(),
            },
            "cache_config": {
                "block_size":
                ...
                "cache_dtype":
                str(getattr(vllm_config.cache_config, "cache_dtype", None)),
                "gpu_memory_utilization":
                getattr(vllm_config.cache_config, "gpu_memory_utilization",
                        None),
            },
        }
    ...
    def start_load_kv(self, forward_context: "ForwardContext",
        ...
        for idx, request in enumerate(metadata.requests):
            ...
            if self.use_layerwise:
                ...
                # A.2b.1: if a routing prior attached a chunk subset for
                # this request — or the debug env var VLLM_LMCACHE_ROUTING_K
                # is set — dispatch to the sparse `retrieve_chunks` API
                # instead of the full-prefix `retrieve`. The mechanism:
                #   1) Walk `process_tokens` over the same (tokens,mask)
                #      the full retrieve would have walked to get the
                #      hash-chain-derived CacheEngineKey for each chunk.
                #   2) Select the subset by positional index.
                #   3) Hand the K-element (keys, starts, ends) list to
                #      `retrieve_chunks`; it delegates to the same
                #      batched_get / batched_to_gpu path that `retrieve`
                #      uses, just with K files instead of N.
                selected_chunk_indices = request.selected_chunk_indices
                if selected_chunk_indices is None:
                    env_k = os.environ.get("VLLM_LMCACHE_ROUTING_K")
                    if env_k is not None:
                        try:
                            k = int(env_k)
                            if k > 0:
                                # Option C (benchmark-only): pick first K
                                # chunks uniformly. This gives us an
                                # empirical TTFT signal without any
                                # per-request routing plumbing.
                                selected_chunk_indices = list(range(k))
                        except ValueError:
                            logger.warning(
                                "VLLM_LMCACHE_ROUTING_K=%s is not an "
                                "integer; ignoring",
                                env_k,
                            )

                if selected_chunk_indices is not None:
                    chunk_infos = list(
                    ...
                    valid_sel = [
                        i for i in selected_chunk_indices
                        if 0 <= i < num_chunks
                    ]
                    if not valid_sel:
                        logger.debug(
                            "A.2b.1 sparse retrieve: req=%s has no valid "
                            "chunk indices in [0,%d); skipping load",
                            request.req_id,
                            num_chunks,
                        )
                    else:
                        sel_starts = [chunk_infos[i][0] for i in valid_sel]
                        sel_ends = [chunk_infos[i][1] for i in valid_sel]
                        sel_keys = [chunk_infos[i][2] for i in valid_sel]
                        ret_mask_list = self.lmcache_engine.retrieve_chunks(
                            keys=sel_keys,
                            starts=sel_starts,
                            ends=sel_ends,
                            kvcaches=kvcaches,
                        ...
                        num_hits = sum(1 for b in ret_mask_list if b)
                        logger.info(
                            "A.2b.1 sparse retrieve: req=%s "
                            "selected=%d/%d chunks hit=%d",
                            request.req_id,
                            len(valid_sel),
                            num_chunks,
                            num_hits,
                        )
                    # Sparse path taken — do NOT fall through to the
                    # full `retrieve`. Advance to the next request.
                    continue
                ...
                num_expected_tokens = (lmcache_cached_tokens -
                                       request.load_spec.vllm_cached_tokens)
    ...
    def wait_for_layer_load(self, layer_name: str) -> None:
        ...
        if self.layerwise_retrievers:
            logger.debug("Waiting for layer %s to be loaded",
                         self.current_layer)
    ...
    def wait_for_save(self):
        ...
        for req_id in connector_metadata.lookup_requests_in_step:
            self.lmcache_engine.lookup_unpin(req_id)  # type: ignore
        ...
        for request in connector_metadata.requests:
            save_spec = request.save_spec
            if (save_spec is None or
                    not save_spec.can_save) and self.kv_role != "kv_producer":
                continue
            ...
            if is_last_prefill:
                ...
                aligned_token_len = (token_len // self._lmcache_chunk_size *
                                     self._lmcache_chunk_size)
    ...
    def get_num_new_matched_tokens(
        ...
        if mm_hashes and mm_positions:
            ...
            apply_mm_hashes_to_token_ids(token_ids_tensor, mm_hashes,
                                         mm_positions)
            token_ids = token_ids_tensor.tolist()
        ...
        lookup_id = request.request_id if self.async_loading else str(
            uuid.uuid4())
    ...
    def update_state_after_alloc(self, request: "Request",
        ...
        kv_transfer_params = (request.kv_transfer_params if hasattr(
            request, "kv_transfer_params") else None)
        ...
        if (self.load_specs[request.request_id].lmcache_cached_tokens
                != request.num_tokens):
            assert (
    ...
    def request_finished(
        ...
        params = (request.kv_transfer_params if hasattr(
            request, "kv_transfer_params") else None)
        return_params = None
