...
class FlashAttentionMetadataBuilder(AttentionMetadataBuilder[FlashAttentionMetadata]):
    ...
    def build(
        ...
        def schedule(
            ...
            if is_quantized_kv_cache(cache_dtype_for_schedule):
    ...
    def forward(
        ...
        if isinstance(kv_cache, tuple):
            raise RuntimeError(
                ...
                "this backend does not support mixed K/V dtypes.  Re-run "
                "with --attention-backend FLASHINFER (or "
                "attention_config.backend=FLASHINFER) for asymmetric KV "
                "caches."
            )
