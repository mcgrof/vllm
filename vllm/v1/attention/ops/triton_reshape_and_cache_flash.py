...
def _fp8_min_max() -> tuple[float, float]:
    """Return (min, max) finite values for FP8_DTYPE.

    Inlined replacement for ``quant_utils.get_fp8_min_max`` (added in a
    later vLLM version than this branch's base) so this kernel module
    does not depend on that helper.
    """
...
def reshape_and_cache_kernel_flash(
    ...
    if USE_HEAD_MAJOR_LAYOUT:
        ...
        tgt_idx_v = (block_idx * block_stride + cur_head * head_stride +
                     cur_dim * dim_stride_v + block_offset * 1)
        # Key addressing (5D): [Block, Head, Dim//8, Slot, 8]
        tgt_idx_k = (block_idx * block_stride + cur_head * head_stride +
                     (cur_dim // x) * dim_stride_k + block_offset * x +
                     (cur_dim % x))
    else:
        cur_head = tile_pos // head_size
        cur_dim = tile_pos % head_size
        tgt_idx_k = (block_idx * block_stride + block_offset * page_stride +
                     cur_head * head_stride + cur_dim)
        tgt_idx_v = tgt_idx_k
...
def _reshape_cache_per_token_head(
    ...
    HEAD_SIZE_PADDED: tl.
    constexpr,  # next_power_of_2(max(head_size, head_size_v))
    ...
    tl.store(
        k_scale_cache_ptr + blk * stride_ks_blk +
        slot_in_blk * stride_ks_slot + head * stride_ks_head,
        k_scale,
    )

    k_q = tl.clamp(k_h * (1.0 / k_scale), QUANT_MIN, QUANT_MAX)
    tl.store(
        key_cache_ptr + blk * stride_kc_blk + slot_in_blk * stride_kc_slot +
        head * stride_kc_head + dim_offs,
        k_q,
        mask=k_mask,
    )
    ...
    tl.store(
        v_scale_cache_ptr + blk * stride_vs_blk +
        slot_in_blk * stride_vs_slot + head * stride_vs_head,
        v_scale,
    )

    v_q = tl.clamp(v_h * (1.0 / v_scale), QUANT_MIN, QUANT_MAX)
    tl.store(
        value_cache_ptr + blk * stride_vc_blk + slot_in_blk * stride_vc_slot +
        head * stride_vc_head + dim_offs,
        v_q,
        mask=v_mask,
    )
...
def triton_reshape_and_cache_flash_per_token_head_quant(
    ...
    Tensor,  # [num_blocks, block_size, num_kv_heads, head_size]
        value_cache: torch.
    Tensor,  # [num_blocks, block_size, num_kv_heads, head_size_v]
        k_scale_cache: torch.
    Tensor,  # [num_blocks, block_size, num_kv_heads] float32
        v_scale_cache: torch.
    Tensor,  # [num_blocks, block_size, num_kv_heads] float32
...
def triton_reshape_and_cache_flash(
    ...
    kv_cache_torch_dtype = (current_platform.fp8_dtype()
                            if is_quantized_kv_cache(kv_cache_dtype) else
                            key_cache.dtype)
