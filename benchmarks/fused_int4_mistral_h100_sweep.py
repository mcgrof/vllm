#!/usr/bin/env python3
"""Mistral H100 fused INT4 policy sweep.

Combined MSL fine-sweep + asymmetric/group_size validation.
Mirrors the Qwen H100 sweep methodology exactly.
...
def main():
    ...
    # Find the lowest MSL with perfect score
    perfect_msl = None
    for msl in msl_values:
    ...
    phase1_path = os.path.join(outdir, f"mistral_msl_fine_sweep_{ts}.json")
    phase1_data = {
        "model": MODEL,
    ...
    # Test at the threshold found and the two values below it
    ...
    phase2_path = os.path.join(outdir, f"mistral_asym_sweep_{ts}.json")
    phase2_data = {
        "model": MODEL,
        "timestamp": time.strftime("%Y-%m-%dT%H:%M:%S"),
    ...
    # Check if asym or smaller GS helps at sub-threshold MSLs
